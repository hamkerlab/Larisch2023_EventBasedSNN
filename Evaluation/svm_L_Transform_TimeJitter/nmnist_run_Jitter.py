...
def main():
    ...
    for t in range(len(std_t)):
        ...
        if not os.path.exists('output_svm/drop_event_t_%i'%(t) ):
            os.mkdir('output_svm/drop_event_t_%i'%(t))
        pwd = 'output_svm/drop_event_t_%i'%(t)

        rec_spkc_test_V1 = np.zeros((n_samples_test,n_p*n_p,n_exN))
        labels_test = np.zeros(n_samples_test)
        
        '''TRANSFORM'''
        transform = tonic.transforms.TimeJitter(std=std_t[t], clip_negative=True)
        ...
        np.save('./output_svm/drop_event_t_%i/spkC_V1_test'%(t),rec_spkc_test_V1)
        np.save('./output_svm/drop_event_t_%i/labels_test'%(t), labels_test)
