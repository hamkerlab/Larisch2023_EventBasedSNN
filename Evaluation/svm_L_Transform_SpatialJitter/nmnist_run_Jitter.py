...
def make_pic(cube_this,pwd):
    ...
    plt.savefig(pwd+'/pic'+'/IMAGE',dpi=300)
...
def main():
    ...
    for x in range(len(std_x)):
        ...
        if not os.path.exists('output_svm/drop_event_x_%i_y_%i'%(x,x) ):
            os.mkdir('output_svm/drop_event_x_%i_y_%i'%(x,x))
        pwd = 'output_svm/drop_event_x_%i_y_%i'%(x,x)

        rec_spkc_test_V1 = np.zeros((n_samples_test,n_p*n_p,n_exN))
        labels_test = np.zeros(n_samples_test)
        
        '''TRANSFORM'''
        transform = tonic.transforms.SpatialJitter(sensor_size=data_test.sensor_size, var_x=x,var_y=x, clip_outliers=True)
        ...
        np.save('./output_svm/drop_event_x_%i_y_%i/spkC_V1_test'%(x,x),rec_spkc_test_V1)
        np.save('./output_svm/drop_event_x_%i_y_%i/labels_test'%(x,x), labels_test)
